...
if nargout > 0
    ...
    function app = createApp(ctx)
        ...
        % Public default: Blocked cvCOCO with the published coherent
        % four-group target and bidirectional held-out validation.
        ...
        function createComponents()
            ...
            app.DCOCOMethod = uidropdown(app.PMethod, ...
                'Items',{'Blocked cvCOCO','Interleaved cvCOCO', ...
                'Adaptive COCO','Fixed-target COCO'}, ...
                'Value','Blocked cvCOCO','Enable','off', ...
                'Tooltip',['Blocked cvCOCO uses two contiguous halves; ', ...
                'Interleaved cvCOCO uses odd/even observations. ', ...
            ...
            app.RFast = uiradiobutton(app.BGEcoCalc, ...
                ...
                'Tooltip','Adaptive eCOCO: per-window adaptive target estimation.');
            app.RAccurate = uiradiobutton(app.BGEcoCalc, ...
                'Text','Blocked eCOCO','Value',false, ...
                'Tooltip',['Blocked eCOCO: frozen targets updated at the selected ', ...
                'fraction of the sliding-window width.']);
            app.RInterleaved = uiradiobutton(app.BGEcoCalc, ...
                'Text','Interleaved eCOCO','Value',false, ...
                'Tooltip',['Interleaved eCOCO: within each complete ', ...
        ...
        function onPadEdgeToggle()
            ...
            elseif isInterleaved
                ...
                % or even observations. Interleaved eCOCO uses complete
        ...
        function updateCocoTargetMode()
            switch app.DCOCOMethod.Value
                case {'Blocked cvCOCO','cvCOCO — Blocked','cvCOCO'}
                    app.cocoTargetMode = 'cv9b';
                case {'Interleaved cvCOCO','cvCOCO — Interleaved'}
        ...
        function onRun()
            ...
            publicAnalysisName = currentPublicAnalysisName();
            invalidateRunState();
            ...
            try
                ...
                isInterleavedEcoRun = app.mode == 2 && ...
                    app.ecocoCalcMode == 3;
                % Keep the older internal token as a compatibility alias
                % for the Blocked cvCOCO group-band implementation.
                isCv9BRun = isCvRun && ...
                ...
                if app.mode == 1
                    if isCvRun
                        ...
                        elseif isLegacyCvRun
                            ...
                            cv.engine = 'compatibility coherent nine-term held-out engine';
                            cv.analysisRole = ...
                        ...
                        elseif isCv9BRun
                            cv = cvcoco9B(app.dataRaw,app.orbit9,app.pad, ...
                                ...
                                'ProgressFcn',@(fraction,message) ...
                                updateProgressDialog(h,fraction,message));
                            % The internal callable implementation retains
                            % its compatibility name; all output is labeled
                            % Blocked cvCOCO.
                            cv.name = 'Blocked cvCOCO';
                            cv.publicName = 'Blocked cvCOCO';
                            cv.abbreviation = 'Blocked cvCOCO';
                            cv.analysisRole = [ ...
                                'Coherent-nine ', ...
                        ...
                        if isInterleavedCvRun
                            ...
                            if ~isempty(conclusionReport)
                                assignin('base','BlockedCVCOCO_conclusion', ...
                                    conclusionReport);
                            end
                        end
                        [outputFile,~] = ...
                        ...
                        if isAdaptiveCocoTargetMode(app.cocoTargetMode)
                            ...
                            conclusionReport = [];
                            fixedResult = struct('name','Fixed-target COCO', ...
                                'method','Fixed-target COCO','corrCI',corrCI, ...
                    ...
                    if app.ecocoCalcMode == 3
                        % Interleaved eCOCO assigns global odd/even identity on
                ...
                if ~isempty(figureSaveError)
                    figureSaveMessage = publicCocoText( ...
                        figureSaveError.message);
                    warning(publicCocoIdentifier( ...
                        publicAnalysisName,'FigureExportFailed'), ...
                        ['The calculation and numerical outputs were saved, ', ...
                        'but automatic FIG/PDF export failed [%s]: %s'], ...
                        publicCocoIdentifier( ...
                            publicAnalysisName,'FigureExportFailed'), ...
                        figureSaveMessage);
                    fprintf(2,'\n>> %s figure export warning\n%s\n\n', ...
                        publicAnalysisName,figureSaveMessage);
                ...
                if cocoIsNonfatalResolutionError(ME)
                    publicMessage = publicCocoText(ME.message);
                    publicIdentifier = publicCocoIdentifier( ...
                        publicAnalysisName,'ResolutionWarning');
                    warning(publicIdentifier,[ ...
                        ...
                        'subsequent runs remain available.'],publicMessage);
                    fprintf(['\n>> %s resolution warning [%s]\n', ...
                        '   Current method skipped; no blocking dialog ', ...
                        'was opened.\n\n'],publicAnalysisName,publicIdentifier);
                    try
                ...
                publicMessage = publicCocoText(ME.message);
                publicIdentifier = publicCocoIdentifier( ...
                    publicAnalysisName,'AnalysisFailed');
                fprintf(2,'\n>> %s error [%s]\n%s\n\n', ...
                    publicAnalysisName,publicIdentifier,publicMessage);
                safeUiAlert(uiFigureAtStart, ...
                    sprintf('%s failed:\n%s',publicAnalysisName,publicMessage), ...
                    [publicAnalysisName,' error'],'error');
            end
        end
        ...
        function [nm,runIndex] = saveCOCOOutputs(corrCI,corr_h0,modeName)
            ...
            outputStem = [dn,'-COCO'];
            if any(strcmp(modeName, ...
                    {'ADAPTIVECOCO','ADAPTIVECOCO9A','ADAPTIVECOCO9B'}))
                outputStem = [dn,'-Adaptive-COCO'];
            elseif any(strcmp(modeName, ...
                    {'FIXEDTARGETCOCO','FIXEDCOCO9'}))
        ...
        function [nm,runIndex] = saveCVCOCOOutputs(cv,modeName)
            ...
            % Detect the target design from result metadata rather than
            % only the public method name.
            ...
            methodLabel = 'Blocked cvCOCO';
            analysisRole = 'Blocked bidirectional held-out analysis';
            ...
            if isInterleavedOutput
                methodLabel = 'Interleaved cvCOCO';
                analysisRole = [ ...
                    'Coherent-nine bidirectional odd/even ', ...
                    'held-out analysis'];
                trainingLabel = [ ...
            ...
            elseif isCv9AOutput
                methodLabel = 'Blocked cvCOCO';
                analysisRole = [ ...
                    'Blocked bidirectional held-out analysis with a ', ...
                    'per-orbit Rayleigh-peak target design'];
                trainingLabel = [ ...
            ...
            elseif isCv9BOutput
                methodLabel = 'Blocked cvCOCO';
                analysisRole = ...
                    'Coherent-nine bidirectional held-out analysis';
                trainingLabel = [ ...
                    'Four union-band energies de-mixed by a 4-by-4 ', ...
                    'finite-record leakage matrix and exact nonnegative ', ...
                    'least squares'];
        ...
        function [nm,runIndex] = saveECOCOOutputs( ...
            ...
            if isnumeric(out_ep) && ~isempty(out_ep)
            ...
            if isfield(details,'windows') && isstruct(details.windows)
                ...
                if all(isfield(windows,requiredGeometry))
                    ...
                    writecell({'Center_m','Requested_lower_m', ...
                        ...
                        'Sheet','Interleaved_WindowGeometry','Range','A1');
                    writematrix([depth(:),windows.requestedBounds, ...
                        ...
                        'Sheet','Interleaved_WindowGeometry','Range','A2');
                end
            end
            ...
            if isfield(details,'folds') && isstruct(details.folds)
                ...
                if all(isfield(folds,requiredFold))
                    ...
                    writecell({'Depth_m','Odd_spacing_m', ...
                        ...
                        'Sheet','Interleaved_FoldResolution','Range','A1');
                    writematrix([depth(:),folds.spacingOdd(:), ...
                        ...
                        'Sheet','Interleaved_FoldResolution','Range','A2');
                end
            end
        end
        ...
        function params = buildRunParameterTable(modeName,outputFile)
            ...
            params(9,:) = {'', '', 'Number of slices', ...
                sliceParameterValue(modeName), ...
                ['Disabled for Blocked cvCOCO and ', ...
                 'Interleaved cvCOCO'],''};
            ...
            params(21,:) = {'', 'COCO','Target mode',cocoTargetModeName(modeName), ...
                ['Blocked cvCOCO, Interleaved cvCOCO, and Adaptive COCO use ', ...
                 'four-group areas with leakage-matrix NNLS and a coherent ', ...
            ...
            params(24,:) = {'', selectedCocoMethod(modeName), ...
                'Split and validation rule',cvParameterValue(modeName, ...
                cvSplitRuleValue(modeName)), ...
                'For Blocked cvCOCO and Interleaved cvCOCO only',''};
            params(25,:) = {'', selectedCocoMethod(modeName), ...
                'Trained target structure',cvTrainingTargetValue(modeName), ...
                'For Blocked cvCOCO and Interleaved cvCOCO only',''};
            params(26,:) = {'', selectedCocoMethod(modeName), ...
                'Symmetric statistic',cvParameterValue(modeName, ...
                'min(max rho A-to-B, max rho B-to-A)'), ...
                'For Blocked cvCOCO and Interleaved cvCOCO only',''};
            params(27,:) = {'', selectedCocoMethod(modeName), ...
                'Monte Carlo batch size', ...
                cvParameterValue(modeName,app.cvBatchSize), ...
                'For Blocked cvCOCO and Interleaved cvCOCO only',''};
            params(28,:) = {'', 'COCO','Random seed',cocoSeedValue(modeName),'Local RNG; restored after the run',''};
            params(29,:) = {'', selectedCocoMethod(modeName), ...
                'Split depth / fold definition', ...
                cvParameterValue(modeName,cvSplitDefinitionValue(modeName)), ...
                'For Blocked cvCOCO and Interleaved cvCOCO only',''};
            params(31,:) = {'', selectedCocoMethod(modeName), ...
                'Directional p_A',cvResultValue(modeName,'pA'), ...
                cvDirectionalPExplanation(modeName,'A'),''};
            params(32,:) = {'', selectedCocoMethod(modeName), ...
                'Directional p_B',cvResultValue(modeName,'pB'), ...
                cvDirectionalPExplanation(modeName,'B'),''};
            params(33,:) = {'', selectedCocoMethod(modeName), ...
                'Monte Carlo null model',cvParameterValue(modeName, ...
            ...
            params(41,:) = {'', '', 'Maximum displayed data frequency (cycle/m)', ...
                ...
                ['Adaptive COCO, Fixed-target COCO, and eCOCO display ', ...
                 'only; not used by Blocked cvCOCO or ', ...
                 'Interleaved cvCOCO'],''};
        ...
        function name = currentPublicAnalysisName()
            name = 'COCO/eCOCO';
            try
                if app.mode == 2
                    spec = ecoMethodSpec();
                    name = spec.displayName;
                elseif strcmp(app.cocoTargetMode,'icv9b')
                    name = 'Interleaved cvCOCO';
                elseif isCvCocoTargetMode(app.cocoTargetMode)
                    name = 'Blocked cvCOCO';
                elseif any(strcmp(app.cocoTargetMode,{'fixed','fixed9'}))
        ...
        function v = cocoTargetModeName(modeName)
            if strcmp(modeName,'ADAPTIVECOCO')
                v = ['four-group band areas with leakage-matrix ', ...
            ...
            elseif isFullRecordCocoModeName(modeName)
                if any(strcmp(modeName,{'FIXEDCOCO9'}))
                    v = 'coherent nine-term fixed target';
                else
                    v = ['four-group band areas with leakage-matrix ', ...
                        'NNLS in a coherent nine-term adaptive target'];
            ...
            elseif strcmp(modeName,'INTERLEAVEDCVCOCO')
                v = ['four group-band area amplitudes, leakage-', ...
...
function refreshMainListbox(ctx,dirpath)
    ...
    try
        ...
        isDir = false(0,1);
        if ~isempty(d)
            sortMode = getSortMode(ctx,listbox);
...
function textValue = publicCocoText(textValue)
% Prevent internal compatibility entry points from leaking through GUI
% errors generated below the public COCO/eCOCO interface.
textValue = char(string(textValue));
replacements = {
    '(cvCOCO9A|cvCOCO9B|cvCOCO9|cvCOCO2|cvCOCOLegacy)', ...
        'Blocked cvCOCO';
    '(Crossfit-eCOCO9B|Crossfit-eCOCO|crossfit eCOCO|eCOCOCrossfitCore)', ...
        'Blocked eCOCO';
    'eCOCOInterleavedCore','Interleaved eCOCO';
    'eCOCOAdaptiveCore','Adaptive eCOCO';
    '(legacy cvCOCO|B-cvCOCO)','Blocked cvCOCO';
    'I-cvCOCO','Interleaved cvCOCO';
    'B-eCOCO','Blocked eCOCO';
    'I-eCOCO','Interleaved eCOCO';
    'Method[- ]?B','four-group';
    'Method[- ]?A','per-orbit'};
for replacementIndex = 1:size(replacements,1)
    textValue = regexprep(textValue,replacements{replacementIndex,1}, ...
        replacements{replacementIndex,2},'ignorecase');
end
end

function identifier = publicCocoIdentifier(methodName,suffix)
switch char(string(methodName))
    case 'Blocked cvCOCO'
        methodToken = 'BlockedCVCOCO';
    case 'Interleaved cvCOCO'
        methodToken = 'InterleavedCVCOCO';
    case 'Adaptive COCO'
        methodToken = 'AdaptiveCOCO';
    case 'Fixed-target COCO'
        methodToken = 'FixedTargetCOCO';
    case 'Adaptive eCOCO'
        methodToken = 'AdaptiveECOCO';
    case 'Blocked eCOCO'
        methodToken = 'BlockedECOCO';
    case 'Interleaved eCOCO'
        methodToken = 'InterleavedECOCO';
    otherwise
        methodToken = 'COCO';
end
identifier = sprintf('Acycle:%s:%s',methodToken,suffix);
end
