...
% splits a cleaned depth series at its deterministic depth midpoint.  Each
% half is regularized independently at that half's median depth spacing.
% In each direction, one half estimates a best sedimentation rate and the
% requested target amplitudes; those amplitudes are frozen while the other
% half selects its own validation rate. FOUR-GROUP models fit four
...
% The null is conditional on the deterministic, separately regularized
% A/B sample grids; raw irregular sampling is not resimulated.
...
% kyr and must contain exactly 1/4/1/3 periods in these threshold groups:
%   long eccentricity >= 200 kyr
%   short eccentricity >= 80 and < 200 kyr
%   obliquity          >= 30 and < 80 kyr
%   precession          < 30 kyr
%
...
%                 result structure (default 'cvCOCO').
%
...
if isFourGroupTraining
    ...
    fprintf('   Sedimentation-rate model: one constant rate within each A/B segment.\n');
...
[dataClean,dataA,dataB,splitDepth,spacingA,spacingB,interpA,interpB] = ...
    prepareHeldOutData(data,analysisName);
...
if ~resolvedA || ~resolvedB
    error('cvcoco:ZeroVariance', ...
        ['Each held-out segment must retain numerically resolved variance ', ...
         'after linear detrending; constant or affine-only segments are ', ...
         'not valid spectral observations.']);
end
% Each independently regularized half has its own sample spacing and may
% have its own persistence.  Fit one transparent conditional-least-squares
% AR(1) coefficient per half and never treat the A(end)->B(1) boundary as a
% unit-lag observation.
[rhoMA,rhoMethodA] = estimateRhoM(detrendedA);
[rhoMB,rhoMethodB] = estimateRhoM(detrendedB);
rhoM = mean([rhoMA,rhoMB]); % scalar compatibility summary; not simulated
...
if ~any(geomA.validRateMask) || ~any(geomB.validRateMask)
    error('cvcoco:NoValidSedimentationRate', ...
        ['No tested sedimentation rate has usable frequency geometry in ', ...
         'both held-out halves. Use a longer record, widen the rate range, ', ...
         'or revise the sampling interval. Numerically valid rates: ', ...
         'Segment A %d of %d; Segment B %d of %d.'], ...
        nnz(geomA.validRateMask),numel(srGrid), ...
        nnz(geomB.validRateMask),numel(srGrid));
end
if ~any(geomA.trainingRateMask) || ~any(geomB.trainingRateMask)
    if isRayleighPeakTraining
        error('cvcoco:NoAllNineTrainingRate', ...
            ['%s must train all nine separately calibrated Rayleigh-peak ', ...
             ...
             'half. Theoretical all-nine ranges (cm/kyr): Segment A ', ...
             '(%.6g, %.6g], Segment B (%.6g, %.6g]. Widen/refine the ', ...
    ...
    error('cvcoco:NoAllNineTrainingRate', ...
        ['%s must train %s at a tested ', ...
         ...
         'exists in at least one half. Theoretical all-nine ranges ', ...
         '(cm/kyr): Segment A (%.6g, %.6g], Segment B (%.6g, %.6g]. ', ...
         'Continuous cross-group training-band overlaps: Segment A %d ', ...
         'rates; Segment B %d rates. Ill-conditioned leakage matrices ', ...
         '(rcond < %.3g): Segment A %d rates; Segment B %d rates. ', ...
         'Widen/refine the tested grid, use a longer/', ...
         'finer sampled record, or inspect the orbital band geometry.'], ...
        analysisName,trainingTargetDescription, ...
        geomA.allNineRateRange(1),geomA.allNineRateRange(2), ...
        geomB.allNineRateRange(1),geomB.allNineRateRange(2), ...
        nnz(geomA.crossGroupBandOverlap), ...
        nnz(geomB.crossGroupBandOverlap),geomA.minimumLeakageRcond, ...
...
if any(~isfinite(frozenA),'all') || any(~isfinite(frozenB),'all')
    error('cvcoco:InvalidTrainedWeights', ...
        ['At least one training half produced nonfinite frozen target ', ...
...
if nsim > 0
    ...
    bytesPerSimulation = 64*max(1, ...
        nTotal+nFrequencyA+nFrequencyB+2*nRate);
...
if nsimCompleted > 0 && (nsimValid ~= nsimCompleted || ...
    ...
    error('cvcoco:InvalidMonteCarloStatistics', ...
        ['At least one full-pipeline Monte Carlo realization produced a ', ...
         ...
         'Finite symmetric/A-to-B/B-to-A counts: %d/%d/%d of %d.'], ...
        nsimValid,nsimValidAtoB,nsimValidBtoA,nsimCompleted);
end
...
result.pAtoB = pAtoB;
result.pBtoA = pBtoA;
% pA evaluates Segment A held out (B -> A); pB evaluates Segment B held
% out (A -> B).  Direction-named fields above avoid any ambiguity.
result.pA = pBtoA;
result.pB = pAtoB;
...
result.groupLeakageRcondA = geomA.groupLeakageRcond;
result.groupLeakageRcondB = geomB.groupLeakageRcond;
...
result.config = struct( ...
    ...
    'seed',options.Seed,'splitRule','depth midpoint', ...
    ...
    'interpolation','separate linear interpolation at each half''s median spacing', ...
    'nullConditioning', ...
...
if ~isequal(counts,required)
    error('cvcoco:InvalidOrbitGroups', ...
        ['ORBIT9 must contain 1 long-eccentricity period (>=200 kyr), ', ...
         '4 short-eccentricity periods (80-<200 kyr), 1 obliquity ', ...
         'period (30-<80 kyr), and 3 precession periods (<30 kyr). ', ...
         'Detected counts: %d/%d/%d/%d.'],counts(1),counts(2),counts(3),counts(4));
end
groups.index = index;
...
function [clean,A,B,splitDepth,dzA,dzB,infoA,infoB] = ...
        prepareHeldOutData(data,analysisName)
...
rawA = clean(clean(:,1) <= splitDepth,:);
rawB = clean(clean(:,1) > splitDepth,:);
...
A = regularizeHalf(rawA,'A');
B = regularizeHalf(rawB,'B');
dzA = median(diff(A(:,1)));
dzB = median(diff(B(:,1)));
infoA = interpolationInfo(rawA,A,dzA);
infoB = interpolationInfo(rawB,B,dzB);
reportInterpolation(infoA,'A',analysisName);
reportInterpolation(infoB,'B',analysisName);
end
...
if isempty(dz) || ~isfinite(dz) || dz <= 0
    error('cvcoco:InvalidDepthSpacing', ...
        'Segment %s has no positive finite depth spacing.',label);
end
...
if ~isfinite(interpolatedPointCount) || interpolatedPointCount < 4 || ...
    ...
    error('cvcoco:InterpolationGridTooLarge', ...
        ['Segment %s median-spacing interpolation would create ', ...
...
if numel(grid) < 4
    error('cvcoco:InsufficientInterpolatedHalf', ...
        'Half %s contains fewer than four points on the median-spacing grid.',label);
end
...
if size(out,1) < 4
    error('cvcoco:InsufficientInterpolatedHalf', ...
        'Half %s contains fewer than four finite interpolated points.',label);
end
end
...
tolerance = max(64*eps(max(1,max(abs(depth)))),1e-8*max(1,abs(dz)));
end
...
fprintf('\n>> %s preprocessing: uneven depth spacing detected in Segment %s.\n', ...
    analysisName,label);
...
geom.allNineRateRange = [max(200*dz./periods), ...
...
matrix = geom.groupLeakageMatrix{out.bestIndex};
if isequal(size(matrix),[4,4])
    out.groupLeakageMatrix = matrix;
end
out.groupLeakageRcond = geom.groupLeakageRcond(out.bestIndex);
end
