...
%ECOCOCROSSFITCORE Blocked cross-validated evolutionary COCO engine.
%
...
%
% Method-B training normally uses only rates at which all nine orbital
...
if ~any(strictTrainingRateMask) && any(geom.relaxedTrainingRateMask)
    ...
    trainingWarningMessage = sprintf([ ...
        ...
        'resolved partial-orbit Method-B target at %d of %d tested ', ...
    ...
    for firstSimulation = 1:actualBatchSize:nsim
        ...
        if any(~isfinite(nullAnchorBestRate)) || ...
            ...
            error('ecocoCrossfitCore:InvalidMonteCarloTraining', ...
                ['A null anchor failed the effective Method-B training ', ...
...
if strcmp(windowMode,'physical-depth')
    result.algorithmVersion = ...
        'Crossfit-eCOCO9B-physical-window-partial-group-v5';
else
    result.algorithmVersion = ...
...
if partialOrbitFallback
    ...
    inferenceQualification = ...
        'complete all-nine Method-B training geometry';
end
result.metadata = struct( ...
    'algorithm','blocked cross-validated Method-B coherent-nine eCOCO', ...
...
if strcmp(windowMode,'legacy-count') && ...
    ...
    error('ecocoCrossfitCore:UnevenDepth', ...
        ['DATA must already be evenly sampled at DT before cross-fitted ', ...
         'legacy-count window geometry is shared. Use physical-depth ', ...
         'mode for irregularly sampled observations.']);
end
