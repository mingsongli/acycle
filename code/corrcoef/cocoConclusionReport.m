...
validateResultFields(cv,required,'confirmatory Blocked cvCOCO');
...
[degradedMode,trainingCompletenessA,trainingCompletenessB] = ...
    trainingMode(cv);
confirmatoryEligible = ~degradedMode;
if degradedMode
    trainingQualification = sprintf([ ...
        'TRAINING QUALIFICATION: complete nine-period training was ', ...
        'unavailable in at least one segment (Segment A: %s; Segment B: ', ...
        '%s). The numerical pipeline therefore used only resolved orbital ', ...
        'groups and fixed unresolved group weights to zero. This is a ', ...
        'partial-orbit exploratory analysis and is not eligible for a ', ...
        'confirmatory decision.'],trainingCompletenessA, ...
        trainingCompletenessB);
else
    trainingQualification = [ ...
        'Both segments trained the complete nine-period target and are ', ...
        'eligible for confirmatory interpretation.'];
end
...
if degradedMode
    if directionalPass && symmetricPass
        classification = ...
            'Partial-orbit exploratory; directional and symmetric criteria met';
        conclusion = [ ...
            'EXPLORATORY PARTIAL-ORBIT RESULT: both numerical directional ', ...
            'global-p curves cross 0.05 and the secondary symmetric joint ', ...
            'statistic is significant. No confirmatory detection is issued.'];
    elseif directionalPass
        classification = ...
            'Partial-orbit exploratory; directional criterion met';
        conclusion = [ ...
            'EXPLORATORY PARTIAL-ORBIT RESULT: the numerical directional ', ...
            'robustness criterion is met, but the secondary symmetric joint ', ...
            'statistic is not significant. No confirmatory detection is issued.'];
    elseif symmetricPass
        classification = ...
            'Partial-orbit exploratory; joint-only / inconclusive';
        conclusion = [ ...
            'EXPLORATORY PARTIAL-ORBIT RESULT: p_sym is significant, but ', ...
            'at least one directional global-p curve never crosses 0.05. ', ...
            'No confirmatory detection is issued.'];
    else
        classification = 'Partial-orbit exploratory; negative';
        conclusion = [ ...
            'EXPLORATORY PARTIAL-ORBIT RESULT: the bidirectional numerical ', ...
            'global-p criterion is not satisfied. No confirmatory detection ', ...
            'is issued.'];
    end
elseif directionalPass && symmetricPass
...
if allNineAtBothBestRates
    ...
    participationQualification = sprintf([ ...
        ...
        'numerical p_robust statistic is unchanged, but any positive ', ...
        'association applies only to ', ...
        'the resolved partial orbital target, not to all nine periods.'], ...
...
conclusion = [conclusion,' ',trainingQualification,' ', ...
    participationQualification];
...
if degradedMode
    ...
    directionalCriterionLabel = ...
        'Numerical directional global-p criterion (exploratory only)';
    directionalStatus = criterionStatus(directionalPass);
    ...
    directionalStatus = passFail(directionalPass);
    symmetricStatus = passFail(symmetricPass);
end
report.degradedMode = degradedMode;
report.trainingCompletenessA = trainingCompletenessA;
report.trainingCompletenessB = trainingCompletenessB;
report.trainingQualification = trainingQualification;
report.confirmatoryEligible = confirmatoryEligible;
...
report.pass = confirmatoryEligible && directionalPass;
report.classification = classification;
report.conclusion = conclusion;
report.summaryRows = {
    'Conclusion method',report.method;
    'Analysis role',analysisRole;
    'Training completeness: Segment A',trainingCompletenessA;
    'Training completeness: Segment B',trainingCompletenessB;
    'Training qualification',trainingQualification;
    'Eligible for confirmatory interpretation', ...
        passFail(report.confirmatoryEligible);
    ...
    directionalCriterionLabel,directionalStatus;
...
report.message = sprintf([ ...
    ...
    messageHeading,pAtoB,report.bestRateAtoB,pBtoA,report.bestRateBtoA, ...
    periodCountAtoB,periodCountBtoA,resolvablePeriodCountAtoB, ...
    resolvablePeriodCountBtoA,groupOrderText, ...
    mat2str(groupCountsAtoB), ...
    mat2str(groupCountsBtoA), ...
    pRobust,directionalStatus,alpha,pSym, ...
    symmetricStatus,classification,conclusion);
end

function [degradedMode,completenessA,completenessB] = trainingMode(cv)
degradedMode = false;
if isfield(cv,'degradedMode')
    value = cv.degradedMode;
    if ~((islogical(value) || isnumeric(value)) && isscalar(value) && ...
            isreal(value) && isfinite(value) && any(value == [0 1]))
        error('cocoConclusionReport:InvalidDegradedMode', ...
            'degradedMode must be a logical scalar.');
    end
    degradedMode = logical(value);
end
completenessA = trainingCompletenessText(cv,'trainingCompletenessA', ...
    degradedMode);
completenessB = trainingCompletenessText(cv,'trainingCompletenessB', ...
    degradedMode);
end

function value = trainingCompletenessText(cv,fieldName,degradedMode)
fallback = 'complete-nine';
if degradedMode
    fallback = 'partial-orbit';
end
value = fallback;
if ~isfield(cv,fieldName)
    return
end
candidate = cv.(fieldName);
if ~((ischar(candidate) && isrow(candidate)) || ...
        (isstring(candidate) && isscalar(candidate)))
    error('cocoConclusionReport:InvalidTrainingCompleteness', ...
        '%s must be a character vector or scalar string.',fieldName);
end
candidate = strtrim(char(string(candidate)));
...
function text = criterionStatus(tf)
if tf
    text = 'NUMERICAL CRITERION MET';
else
    text = 'NUMERICAL CRITERION NOT MET';
end
end
