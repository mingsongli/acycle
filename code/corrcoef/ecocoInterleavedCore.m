...
%ECOCOINTERLEAVEDCORE Internal engine for Interleaved eCOCO.
%
...
% globally Odd observations train a frozen four-group coherent-nine target
...
for windowIndex = 1:nWindows
    if ~isfinite(starts(windowIndex)) || ~isfinite(ends(windowIndex)) || ...
        ...
        failureIdentifier{windowIndex} = ...
            'Acycle:InterleavedECOCO:InsufficientWindowObservations';
    ...
    try
        ...
        if isUnresolvableWindow(exception)
            failureIdentifier{windowIndex} = ...
                'Acycle:InterleavedECOCO:WindowResolutionUnsupported';
            failureReason{windowIndex} = ...
                publicInterleavedFailureText(exception.message);
            if nsim > 0
                reportTotalMonteCarloWork(windowIndex,1);
            else
                reportProgress(options.ProgressFcn,windowIndex/nWindows, ...
...
if ~any(success)
    error('ecocoInterleavedCore:NoResolvableWindows', ...
        ['Every complete sliding window was frequency-unresolvable for ', ...
         'the requested sedimentation-rate grid. Review the sampling ', ...
         'interval, window width, and sedimentation-rate range.']);
end
...
result.abbreviation = 'Interleaved eCOCO';
result.version = 3;
result.method = 'Interleaved eCOCO';
...
result.algorithmVersion = 'Interleaved eCOCO — windowed v3';
result.metadata = struct( ...
    'algorithm','Interleaved eCOCO', ...
    'experimental',true, ...
...
try
    ...
    warning('Acycle:InterleavedECOCO:ProgressCallbackFailed', ...
        'Progress callback failed: %s',exception.message);
end
clear restoreRandomState
end

function textValue = publicInterleavedFailureText(textValue)
textValue = char(string(textValue));
textValue = regexprep(textValue, ...
    '(cvCOCO9A|cvCOCO9B|cvCOCO9|interleavedcvcoco)', ...
    'Interleaved cvCOCO','ignorecase');
textValue = regexprep(textValue,'Method[- ]?B', ...
    'four-group','ignorecase');
end
