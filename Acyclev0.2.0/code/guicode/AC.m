...
if and ((min(plot_selected) > 2), (nplot == 1))
    ...
    GETac_pwd; data_name = fullfile(ac_pwd,data_name);
        ...
        if sum(strcmp(ext,handles.filetype)) > 0
            ...
            time = data(:,1);
            npts = length(time);
            srmean = median(diff(data(:,1)));
%             nyquist = 1/(2*srmean);
%             rayleigh = 1/(npts*srmean);
            
            prompt = {'DATA: What is the age of the data in Ma (e.g., 55):',...
                'TARGET: MIN frequency (default)',...
            ...
            defaultans = {num2str(handles.t1),num2str(handles.f1),num2str(handles.f2),'5000'};
            options.Resize='on';
            answer = inputdlg(prompt,dlg_title,num_lines,defaultans,options);
            if ~isempty(answer)
                t1 = 1000*str2double(answer{1});
                f1 = str2double(answer{2});
                f2 = str2double(answer{3});
                pad = str2double(answer{4});
                ...
                prompt = {'DATA: sampling rates',...
                    ...
                    'Adjust target power to that of real data : 0 = No, 1 = Yes',...
                    'Split series: 1, 2, 3, ...',...
                    'Correlation method: 1 = Pearson; 2 = Spearman'};
                if t1 >= 249
                ...
                num_lines = 1;
                defaultans = {num2str(srmean),num2str(pad),num2str(handles.sr1),...
                    num2str(handles.sr2),num2str(handles.srstep),num2str(handles.nsim),...
                   num2str(handles.red),num2str(handles.adjust),num2str(handles.slices),'1'};     
               
                options.Resize='on';
                answer = inputdlg(prompt,dlg_title,num_lines,defaultans,options);
                if ~isempty(answer)
                    srm = str2double(answer{1});
                    pad1 = str2double(answer{2});
                    sr1 = str2double(answer{3});
                    sr2 = str2double(answer{4});
                    srstep = str2double(answer{5});
                    nsim = str2double(answer{6});
                    red = str2double(answer{7});
                    adjust = str2double(answer{8});
                    slices = str2double(answer{9});
                    plotn = 1;
                    corrmethod = str2double(answer{10});
                    ...
                    param0 = ['Target age is ~',num2str(t1),' ka. Pad = ',num2str(pad)];
                    param1 = ['Sedimentation rate step = ', num2str(srstep),', from ',num2str(sr1),' to ',num2str(sr2)];
                    param2 = ['Data: number of slices = ', num2str(slices),'. # of simulations = ',num2str(nsim),'. Pad = ',num2str(pad1)];
                    if corrmethod == 1
        ...
        if sum(strcmp(handles.ext,handles.filetype)) > 0
            ...
            if ~isempty(answer)
                ...
                prompt = {'DATA: running window (m)',...
                    ...
                    'DATA: zero-padding number (default)',...
                    'DATA: MIN sedimentation rate (cm/kyr)',...
                    'DATA: MAX sedimentation rate (cm/kyr)',...
                    'DATA: STEP sedimentation rate (cm/kyr)',...
                    'Number of simulations (e.g., 200, 600, 2000)',...
                    'Remove red noise: 0 = No, 1 = x/AR(1), 2 = x-AR(1)',...
                    'Adjust target power to that of real data : 0 = No, 1 = Yes',...
                    'Split series for each sliding window: 1, 2, 3, ...',...
                    'Plot: 1 = 1 fig; 2 = multi-figs; 3 = 3D; Reverse Y-axis = (-1,-2, or -3)'};
                dlg_title = 'Evolutionary COrrelation COefficient: DATA';
                num_lines = 1;
                step = round(npts/50);
                defaultans = {num2str(0.5*(time(npts)-time(1))),num2str(srmean),...
                    num2str(step),num2str(pad),num2str(handles.sr1),num2str(handles.sr2),...
                    num2str(handles.srstep),num2str(handles.nsim),num2str(handles.red),...
                    num2str(handles.adjust),num2str(handles.slices),'1'};
                
                options.Resize='on';
                answer = inputdlg(prompt,dlg_title,num_lines,defaultans,options);
                if ~isempty(answer)
                    window = str2double(answer{1});
                    srm = str2double(answer{2});
                    step = str2double(answer{3});
                    pad1 = str2double(answer{4});
                    sr1 = str2double(answer{5});
                    sr2 = str2double(answer{6});
                    srstep = str2double(answer{7});
                    nsim = str2double(answer{8});
                    red = str2double(answer{9});
                    delinear = 0;
                    adjust = str2double(answer{10});
                    slices = str2double(answer{11});
                    plotn = str2double(answer{11});
                    disp('>> Wait ...')
                    ...
                    param0 = ['Target age = ',num2str(t1),' ka. Frequency from ',num2str(f1),' to ',num2str(f2),' cycles/kyr. Pad = ',num2str(pad)];
                    param1 = ['Sliding window = ',num2str(window),'. Detrend = ',num2str(delinear),'. Number of step = ', num2str(step),'.'];
                    param2 = ['Tested sed. rate from ',num2str(sr1),' to ',num2str(sr2),' with a step = ', num2str(srstep), ' cm/kyr.'];
                    param3 = ['# Simulations = ',num2str(nsim), '. Remove red noise = ',num2str(red),'. Adjust target = ',num2str(adjust),'. Pad = ',num2str(pad1)];
                    disp('')
                    disp(' - - - - - - - - - - - - - Summary - - - - - - - - - - - ')
                    disp(data_name);
                    disp(param0);
                    ...
                    if exist([pwd,handles.slash_v,log_name])
                        for i = 1:100
                            log_name = [dat_name,'-',num2str(nsim),'sim-',num2str(slices),'slice-',num2str(window),'win-ECOCO-log',num2str(i),ext];
                            if exist([pwd,handles.slash_v,log_name])
                            else
                                break
                            end
